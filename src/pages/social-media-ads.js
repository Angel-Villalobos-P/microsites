...
const SocialMediaAds = () => {
  ...
  useEffect(() => {
    fetch("/.netlify/functions/socialMediaAdsContent")
      .then(res => res.json())
  ...
  return (
    <Layout>
      ...
      {content && (
        <TwoColumnsAndBlackbox item={content?.zones?.MainContentZone[2]} />
      )}
      {content && (
        <TwoColumnsLeftContent item={content?.zones?.MainContentZone[3]} />
      )}
