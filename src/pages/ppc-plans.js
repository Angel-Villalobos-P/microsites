...
const PPCPlans = ({ serverData }) => {
  ...
  return (
    <Layout>
      ...
      {MainContentZone && (
        <TwoColumnsContentAndList item={MainContentZone[3]} />
      )}
      {MainContentZone && <TableBlock item={MainContentZone[4]} />}
      {MainContentZone && <GridListing item={MainContentZone[5]} />}
      {MainContentZone && (
        <TwoColumnsContentAndList item={MainContentZone[6]} />
      )}
      {MainContentZone && (
        <TwoColumnsLeftListAndImage item={MainContentZone[7]} />
      )}
...
export async function getServerData() {
  const rest = await fetch(
    "https://api.aglty.io/800c8044/fetch/en-us/page/101?contentLinkDepth=3&expandAllContentLinks=true",
    {
      headers: {
        APIKEY:
